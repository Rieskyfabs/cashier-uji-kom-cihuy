                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <ul class="list-group">
                                                @foreach ($products as $key => $product)
                                                    <li class="list-group-item">
                                                        ...
                                                        <br>Subtotal: Rp
                                                        {{ number_format($product->price * $filteredQuantities[$product->id], 0, ',', '.') }}
                                                    </li>
                                                    <hr>
                                                @endforeach
                                            </ul>
                                            ...
                                            <input type="hidden" name="product_data"
                                                value="{{ json_encode(
                                                    $products->map(function ($product) use ($filteredQuantities) {
                                                        return [
                                                            'id' => $product->id,
                                                            'name' => $product->name,
                                                            'price' => $product->price,
                                            ...
                                            <div class="form-group mb-3" id="member_selection" style="display: none;">
                                                ...
                                                <select class="form-control select2" id="member_phone" name="member_id">
                                                    ...
                                                    @foreach ($members as $member)
                                                        <option value="{{ $member->id }}">{{ $member->phone_number }} -
                                                            {{ $member->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
