                                        ...
                                        @foreach ($sales as $index => $item)
                                            ...
                                            <td class="text-center">
                                                <button type="button" class="btn btn-primary detail-transaction-btn"
                                                    ...
                                                    data-transaction='{{ json_encode($item) }}'>Lihat</button>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.detail-transaction-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    let products = typeof data.product_data === "string" ? JSON.parse(data
                    ...
                    document.getElementById('paymentAmount').textContent = (data.payment_amount ||
                        0).toLocaleString('id-ID');
                    document.getElementById('changeAmount').textContent = (data.change_amount || 0)
                        .toLocaleString('id-ID');
                    document.getElementById('discountAmount').textContent = (totalProductPrice -
                        data.total_amount || 0).toLocaleString('id-ID');
                });
            });
        });
    </script>
